...
class IndexTTS2:
    ...
    def infer(self, spk_audio_prompt, text, output_path,
        ...
        if use_emo_text or emo_vector is not None: ## default is false
            # we're using a text or emotion vector guidance; so we must remove
            # "emotion reference voice", to ensure we use correct emotion mixing!
            emo_audio_prompt = None

        if use_emo_text: ## default is false
        ...
        if emo_vector is not None: # default is false
        ...
        if emo_audio_prompt is None: # default is true
        ...
        if self.cache_spk_cond is None or self.cache_spk_audio_prompt != spk_audio_prompt: #default is true
            if self.cache_spk_cond is not None: #default is false
                self.cache_spk_cond = None
                self.cache_s2mel_style = None
                self.cache_s2mel_prompt = None
                self.cache_mel = None
        ...
        if self.cache_emo_cond is None or self.cache_emo_audio_prompt != emo_audio_prompt: # default is true
        ...
        segments = self.tokenizer.split_segments(text_tokens_list, max_text_tokens_per_segment) # default 120 tokens
...
#    text = '欢迎来到中国，你看中什么礼物了？这是我的中学。他是中路军的士兵。恭喜你中奖了'
    ...
    #text = 'Emma and Dan in particular have both made it clear over the last few years that they think our former professional association gives them a particular right - nay, obligation - to critique me and my views in public, she wrote, also referring to Daniel Radcliffe, who played the titular character in the film. Both Radcliffe and Watson, as well as Rupert Grint, who played Ron Weasley in the films, have distanced themselves from Rowlings anti-trans viewpoints over the past decade. Years after they finished acting in Potter, they continue to assume the role of de facto spokespeople for the world I created, she continued. When you have known people since they were ten years old it is hard to shake a certain protectiveness. Until quite recently, I had not managed to throw off the memory of children who needed to be gently coaxed through their dialogue in a big scary film studio. For the past few years, I have repeatedly declined invitations from journalists to comment on Emma specifically.'
...
if __name__ == "__main__":
    ...
    """
    speakers = ["examples/voice_01.wav", "examples/voice_11.wav"]
    sentences = [
        "欢迎来到中国，你看中什么礼物了？这是我的中学。他是中路军的士兵。恭喜你中奖了",
        "欢迎大家来体验indextts2，并给予我们意见与反馈，谢谢大家。"
    ...
    audio_dir, text_dir, output_dir = sys.argv[1:4] 
    ...
    sentences = []
    for f in textfiles:
        with open(text_dir+"/"+f, 'r') as file:
    ...
    for loop in range(1,10):
        ...
        for spk_idx, spk_audio in enumerate(speakers):
            #for emo_idx, emo in enumerate(emotions):
            for emo_idx,emo in enumerate(emo_vectors):
                starti = loop*100+spk_idx*5 
                #for sent_idx, text in enumerate(sentences):
                for i in range(starti,min(starti+10,len(textfiles))):
                    txt = textfiles[i]

                    spk_str = spk_audio.split('.')[-2].split('_')[-1]
                    txt_str = txt.split('.')[-2].split('_')[-1]

                    #out_name = f"speaker{spk_idx:02d}_sentence{sent_idx:03d}_emo{emo_idx}.wav"
                    out_name = f"{spk_str}_{txt_str}_emo{emo_idx}.wav"
                    ...
                    tts.infer(
                        spk_audio_prompt=audio_dir+"/"+spk_audio,
                        text=text,
