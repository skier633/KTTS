...
def build_model(cfg_path: Path, tokenizer: TextTokenizer, base_checkpoint: Path, device: torch.device) -> UnifiedVoice:
    ...
    lora.apply_lora_to_gpt(model,8,16)#version4
    #lora.apply_lora_to_finalnorm(model,8,16)#version4
...
def main() -> None:
    ...
    for name, param in model.named_parameters():
        if "lora_" in name or "ln_f" in name or "final_norm" in name:
            param.requires_grad = True
            lora_params.append(param)
        else:
            param.requires_grad = False

    ## debug information version4e
    '''
    print(len(lora_params)," params are trainable")
    for name, param in model.named_parameters():
        print(f"Layer: {name} | Size: {param.size()} | {param.requires_grad}")

    print(f"Type of gpt.ln_f: {type(model.gpt.ln_f)}")
    print(f"Type of final_norm: {type(model.final_norm)}")
    '''

    model.gpt.gradient_checkpointing_enable()

    # 3. Ensure the output head is also "aware" (optional but recommended)
    if hasattr(model.gpt, 'lm_head'):
        model.gpt.lm_head.weight.requires_grad = False # Keep this frozen

    #version4e
    if hasattr(model.gpt, "gradient_checkpointing_disable"):
        model.gpt.gradient_checkpointing_disable()
    #model.gpt.config.use_cache = False # Sometimes required for training
    #model.gpt.config.gradient_checkpointing = False

    #print(len(lora_params)," params are trainable")
    optimizer = AdamW(lora_params, lr=args.learning_rate, weight_decay=args.weight_decay) #version4
